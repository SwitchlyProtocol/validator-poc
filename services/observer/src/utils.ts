...
export function extractOpsData(ops_data: any[]): ExtractedData | null {
  const manageDataRecord = ops_data.find(
    (record) => record.type === "manage_data" && record.name === "switch"
  );
...
function findMemoInEthTxData(input: string) {
  // Find the index of the substring "SWITCH:"
  const startIndex = input.indexOf("SWITCH:");

  // If "SWITCH:" is found, extract the substring starting from the next character
  if (startIndex !== -1) {
    const extractedSubstring = input.substring(startIndex + 7); // 7 is the length of "SWITCH:"
    return extractedSubstring;
  } else {
    // If "SWITCH:" is not found, return null or handle accordingly
    return null;
  }
}
